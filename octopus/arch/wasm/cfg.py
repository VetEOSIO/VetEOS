...
class WasmCFG(CFG):
    ...
    def run_static_analysis(self):
        ...
        #print(func.name)
        #print('\n'.join(['%s:%s' % item for item in func.__dict__.items()]))
        bc=func.basicblocks[0]
        #print('\n'.join(['%s:%s' % item for item in bc.__dict__.items()]))
        ins=bc.instructions[0]
        #print('\n'.join(['%s:%s' % item for item in ins.__dict__.items()]))
        imm=ins.imm_struct
        #print('\n'.join(['%s:%s' % item for item in imm.__dict__.items()]))
        #exit(0)
        ...
        for idx, func in enumerate(self.functions):
            ...
            self.edges += edges
            # print(idx)'''
            '''if func.name != '$func121':
                continue
            for bc in func.basicblocks:
    ...
    def visualize_call_flow(self, filename="wasm_call_graph_octopus.gv",
        ...
        with g.subgraph(name='global') as c:
            ...
            try:
                indirect_target = [self.analyzer.func_prototypes[index][0]
