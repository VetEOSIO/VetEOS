...
class WasmSSAEmulatorEngine(EmulatorEngine):
    ...
    def emulate_one_function(self, function_name, state=WasmVMstate(), depth=0):
        ...
        '''logging.warning("In emul_one_func():\n[+] current_function detected - %x: %s/%s",
                        self.current_function.start_offset,
                        self.current_function.name,
                        self.current_function.prefered_name)'''
        # filter the func60
        '''if self.current_function.name == '$func60':
            logging.warning("func60 detected, break")
            exit(0)'''
        ...
        # print(state.details())
        # logging.warning("End of emul_one_func(), return from emulate()")
    ...
    def emulate(self, state=WasmVMstate(), depth=0):
        ...
        while not halt:
            ...
            if halt:
                # print(state.details())
                if state.details()['ssa_stack']:
                    for i in state.details()['ssa_stack']:
    ...
    def emul_control_instr(self, instr, state, depth):
        ...
        if instr.name == 'unreachable':
            ...
            target = next(filter(lambda element: element.offset ==
                          jump_addr[0], self.current_f_instructions))
    ...
    def emul_parametric_instr(self, instr, state):
        if instr.name == 'drop':
            ...
            instr.ssa = SSA(new_assignement=self.ssa_counter,
                            method_name=instr.name, args=arg)
            state.ssa_stack.append(instr)
            self.ssa_counter += 1
        return False
